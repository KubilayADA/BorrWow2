...
- **List Items:** 
    Users can  create, edit, favorite and delete items listings.
- **Browse Items:** 
...
- **Request to Borrow:** Users can request to borrow available items with specified pickup and return dates.
- **Request History:** Borrowes and lenders can view and manage their borrowing requests.
...
These endpoints handle user authentication, including registration, login, and token verification.
...
Error: 
    { error: "Invalid referral code" } or validation errors.
Relationships:
    If a valid referralCode is provided, the referring user (referredBy) and referred users (referredUsers) are updated in the User model.
Trust points are awarded to both the referrer and the new user.
...
Response:
    Success: { token: <JWT> }
Error: 
    { message: "Incorrect password" } or { message: "No user with this username" }.
Files:
...
Response:
Success: 
    { userId: <userId>, message: "Token valid" }
Error: 
...
These endpoints handle item-related operations, such as creating, updating, and deleting items.
...
These endpoints handle borrowing request-related operations, such as creating, updating, and deleting borrowing requests.
...
### Trust Points
These endpoints handle the redemption of trust points for rewards.
